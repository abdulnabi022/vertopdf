...
export default function PdfToJpg() {
  ...
  const onConvert = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5050/api/pdf-to-jpg', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className={`relative min-h-screen flex flex-col items-center justify-center px-2 py-8 bg-gradient-to-br from-blue-50 to-white transition-all duration-300 ${dragActive ? 'ring-4 ring-blue-400/60' : ''}`}
      ...
      <Card
        title={<span className="flex items-center gap-2 justify-center text-3xl font-extrabold text-blue-900"><DocumentArrowDownIcon className="h-8 w-8 text-pink-500" />PDF to JPG</span>}
        description={<span className="text-lg text-neutral-600">Convert every page of your PDF into high-quality JPG images. Download as a ZIP archive.</span>}
        buttonText={busy ? 'Converting…' : 'Convert to JPG'}
      ...
      >
        ...
        {downloadUrl && (
          <a className="w-full mt-4 block px-5 py-2 rounded-lg bg-gradient-to-r from-green-400 to-green-600 text-white text-base font-semibold text-center shadow hover:from-green-500 hover:to-green-700 transition" href={downloadUrl} download={`pdf-to-jpg_${Date.now()}.zip`}>
            Download JPGs (ZIP)
          </a>
        )}
      </Card>
