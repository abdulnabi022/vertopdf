      ...
      {dragActive && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-blue-100/80 pointer-events-none animate-fade-in">
          <div className="flex flex-col items-center gap-4">
            <svg className="w-16 h-16 text-blue-500 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5m0 0l5-5m-5 5V4" /></svg>
            <span className="text-2xl font-bold text-blue-700 drop-shadow">Drop your PDF here</span>
          </div>
        </div>
      )}
      <Card
        title={<span className="flex items-center gap-2 justify-center text-3xl font-extrabold text-blue-900"><ArrowDownTrayIcon className="h-8 w-8 text-green-600" />Compress PDF</span>}
        description={<span className="text-lg text-neutral-600">Reduce your PDF file size with high-quality compression. Fast. <span className="text-blue-600 font-semibold"></span></span>}
        buttonText={busy ? 'Compressing…' : 'Compress PDF'}
        onButtonClick={onCompress}
        disabled={busy || !file}
      >
        <label
          ref={dropRef}
        ...
        {downloadUrl && (
          <a className="w-full mt-4 block px-5 py-2 rounded-lg bg-gradient-to-r from-green-400 to-green-600 text-white text-base font-semibold text-center shadow hover:from-green-500 hover:to-green-700 transition" href={downloadUrl} download={`compressed_${Date.now()}.pdf`}>
