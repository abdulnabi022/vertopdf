      ...
      {dragActive && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-blue-100/80 pointer-events-none animate-fade-in">
          <div className="flex flex-col items-center gap-4">
            ...
            <span className="text-2xl font-bold text-blue-700 drop-shadow">Drop your PNGs here</span>
          </div>
        </div>
      )}
      <Card
        title={<span className="flex items-center gap-2 justify-center text-3xl font-extrabold text-blue-900"><DocumentArrowUpIcon className="h-8 w-8 text-blue-600" />PNG to PDF</span>}
        description={<span className="text-lg text-neutral-600">Combine multiple PNG images into a single PDF. Drag or select images in order.</span>}
        ...
        >
          ...
          <div className={`w-full h-56 flex flex-col items-center justify-center gap-4 border-2 border-dashed rounded-2xl p-6 text-center bg-white/80 hover:bg-blue-50 transition cursor-pointer shadow-lg ${files.length ? 'border-blue-400' : 'border-blue-200'}`}>
            ...
            <span className="flex items-center gap-2 justify-center text-base text-blue-500">
              ...
              Drag & Drop to select your PNGs
            </span>
            <span className="mt-2 text-blue-900 font-semibold text-base">{files.length ? `${files.length} PNG(s) selected` : 'No files selected'}</span>
          </div>
        </label>
        {downloadUrl && (
          <a className="w-full mt-4 block px-5 py-2 rounded-lg bg-gradient-to-r from-green-400 to-green-600 text-white text-base font-semibold text-center shadow hover:from-green-500 hover:to-green-700 transition" href={downloadUrl} download={`png-to-pdf_${Date.now()}.pdf`}>
            Download PDF
          </a>
        )}
      </Card>
